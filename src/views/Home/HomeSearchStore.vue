...
const store_id_list: Ref<string[]> = ref([])
...
const LoadMerchandiseId = async () => {
    ...
    store_id_list.value = Array.isArray(myArray)
...
// Computed properties
const totalPages = computed(() => Math.ceil(store_id_list.value.length / itemsPerPage.value));
const pagedItems = computed(() => {
  const start = (currentPage.value - 1) * itemsPerPage.value;
  const end = start + itemsPerPage.value;
  return store_id_list.value.slice(start, end);
});
...
<template>
  <div class="home-page">
    ...
    <div class="home-body">
      <div class="merchandise-card" v-for="(seller_id, id) in pagedItems" :key="`${id}-${currentPage}`">
